...
# ═══════════════════════════════════════════════════════════
# 模板 Y：文本清洗纯函数 —— "确定性 + 输入输出表 + 单元测试"
# 适用：任何需要清洗/规范化文本的场景（模型输出、爬虫数据、用户输入）
# ═══════════════════════════════════════════════════════════
#   def normalize_markdown(text: str) -> str:
#       """把毛坯文本整理成规范 Markdown（纯函数：无副作用、同输入同输出）。"""
#       lines = [line.rstrip() for line in text.split("\n")]   # ① 去行尾空白
#       result: list[str] = []
#       for line in lines:                                     # ② 压缩连续空行
...
#               result.insert(i, "")
#               i += 1        # 跳过刚插入的空行（insert 改变列表长度）
#           i += 1
#       return "\n".join(result).strip()
#
# 设计要点：
# 1. 纯函数 = 最好测：准备输入 → 调用 → 断言输出（不用数据库/mock）
# 2. 每条规则 = 一个可测行为（测试用例就是"输入输出对照表"的翻译）
# 3. 只清洗新数据、不动历史数据——避免破坏已有缓存/已存内容
